...
//	}
	...
	public Service getServiceBySCPDURL(String searchUrl)
	{
		ServiceList serviceList = getServiceList();
		int serviceCnt = serviceList.size();
		Debug.message("searchUrl " + searchUrl);
		Debug.message("service List size " + serviceCnt);
		for (int n=0; n<serviceCnt; n++) {
			Service service = serviceList.getService(n);
			Debug.message("service list " + service.getSCPDURL());
		...
		for (int n=0; n<devCnt; n++) {
			...
			Debug.message("device list " + service.toString());
			if (service != null)
				return service;
		}
		Debug.message("cannot find " + searchUrl);
		return null;
	}
	...
	private void httpGetRequestRecieved(HTTPRequest httpReq)
	{
		...
		Debug.message("res is " + httpRes.toString());
