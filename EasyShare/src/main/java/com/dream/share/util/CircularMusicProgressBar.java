...
public class CircularMusicProgressBar extends ImageView {
    ...
    public void setValueDirect(float newValue){
    ...
    public void setBorderProgressColor(int borderColor) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if(mHoldMode&&(ev.getAction() != MotionEvent.ACTION_UP))
			return false;
        ...
        case MotionEvent.ACTION_DOWN:
        	mGestureHandler.sendEmptyMessageDelayed(LONG_PRESS, 1000);
        	mHoldMode = true;
        	mLastFocusX = focusX;
        	Log.d(TAG, "MotionEvent.ACTION_DOWN");
        	return true;
        case MotionEvent.ACTION_MOVE:
        	if(mLastFocusX > focusX){
        		if(mLastFocusX - focusX > 20){
        			mGestureHandler.sendEmptyMessage(MOVE_LEFT);
        			mLastFocusX = focusX;
        		}
        	}else {
        		if(focusX - mLastFocusX > 20){
        			mGestureHandler.sendEmptyMessage(MOVE_RIGHT);
        			mLastFocusX = focusX;
        		}
        	}
            return true;
        case MotionEvent.ACTION_UP:
        	...
        	mGestureHandler.sendEmptyMessage(RELEASE_PRESS);
        	mHoldMode = false;
    ...
    private class GestureHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {   
            	case LONG_PRESS:
            		mHoldMode = false;
	            	mOnGestureListener.onLongPress();
	                break;
	            case RELEASE_PRESS:
	            	mOnGestureListener.onRelease();
	                break;
	            case MOVE_LEFT:
	            	mOnGestureListener.onMoveLeft();
	                break;
	            case MOVE_RIGHT:
	            	mOnGestureListener.onMoveRight();
	                break;
	            default:
