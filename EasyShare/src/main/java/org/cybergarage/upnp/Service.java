...
public class Service {
	...
	private Node getSCPDNode() {
		...
		if (rootDev == null)
			return null;
		Debug.message("rootDev " + rootDev.getDeviceType());
	...
	public ActionList getActionList() {
		...
		Debug.message("nNode " + nNode);
		for (int n = 0; n < nNode; n++) {
			Node node = actionListNode.getNode(n);
			Debug.message("isActionNode " + Action.isActionNode(node));
	...
	public Action getAction(String actionName) {
		...
		Debug.message("action cnt " + actionList.size());
		for (int n = 0; n < nActions; n++) {
			Action action = actionList.getAction(n);
			String name = action.getName();
			Debug.message("action" + name);
			if (name == null)
				continue;
